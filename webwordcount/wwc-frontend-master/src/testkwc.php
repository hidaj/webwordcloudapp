<?php
    ...
    $kwcURL = "http://keywordcount.40313770.qpc.hal.davecutting.uk/";

    function get_http_response_code($kwcURL) {
        $headers = get_headers($kwcURL);
        return substr($headers[0], 9, 3);
    }
    
        $status = get_http_response_code($kwcURL);
        $expected = 200;
        $expectedc = 404;
        $expecteds = 500;
        $clienterror = 404;
